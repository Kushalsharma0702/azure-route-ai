...
const Signup = () => {
  ...
  const handleSignup = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      localStorage.setItem("routeaura_user", JSON.stringify({ email, name }));
      toast.success("Account created! Welcome to RouteAura");
      navigate("/");
    }, 1200);
  };
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        ...
        >
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full h-12 bg-primary text-primary-foreground text-primary-foreground border-0 rounded-xl font-semibold shadow-lg hover:opacity-90 transition-opacity">
              {loading ? (
