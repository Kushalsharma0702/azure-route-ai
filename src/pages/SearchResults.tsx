              ...
              <div className="flex-1 space-y-4">
                {loading ? (
                  ...
                  searchResults.map((result, i) => (
                    <Link to={`/detail/${result.id}`} key={result.id} className="block">
                      <motion.div
                        initial={{ opacity: 0, y: 15 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: i * 0.08 }}
                        whileHover={{ y: -2, boxShadow: "0 8px 30px hsl(230 80% 56% / 0.12)" }}
                        className="bg-card rounded-2xl shadow-card border border-border/50 overflow-hidden flex flex-col md:flex-row transition-all cursor-pointer"
                      >
                        <div className="md:w-72 h-48 md:h-auto flex-shrink-0 overflow-hidden">
                          <img src={result.image} alt={result.name} loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                        </div>
